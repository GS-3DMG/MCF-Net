...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf):
        ...
        self.conv_hd = nn.Conv2d(self.in_dim, self.out_dim, 5, 1, 2)

        #######################################################################
        # Encoder features of hard data
        #######################################################################
        self.down_1_0 = nn.Sequential(
                nn.Conv2d(in_channels=2*self.out_dim, out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim), act_fn,
    ...
    def forward(self, inputs):
        ...
        i0 = inputs[:,0:1,:,:]
        i1 = inputs[:,1:2,:,:]
        # -----  Concatenate Hard Data --------
        n_total = i0.size(2)*i0.size(3)
        num_context = int(torch.empty(1).uniform_(n_total/1000, n_total/100).item())
        sp = i0.new_empty(i0.size(0), 1, i0.size(2), i0.size(3)).bernoulli_(p=num_context/n_total).repeat(1, 1, 1, 1)
        hd = sp*i0
        hd_h = self.conv_hd(hd)
        sp_h = self.conv_hd(sp)
        hd_fmap = torch.cat([hd_h, sp_h], 1)

        # -----  First Level -------- 
        down_1_0 = self.down_1_0(hd_fmap)
    ...
    def complete(self, inputs, sp=None, missing_rate=None):
        i0 = inputs[:, 0:1, :, :]
        i1 = inputs[:, 1:2, :, :]
        if sp is None:
            ...
            sp = i0.new_empty(i0.size(0), 1, i0.size(2), i0.size(3)).bernoulli_(p=1-missing_rate).repeat(1, 1, 1,
                                                                                                                1)
    ...
    def complete_with_feature_output(self, inputs, sp=None, missing_rate=None, save_path=""):
        i0 = inputs[:, 0:1, :, :]
        i1 = inputs[:, 1:2, :, :]
        if sp is None:
            if missing_rate is None:
            ...
            n_total = i0.size(2) * i0.size(3)
            num_context = int(torch.empty(1).uniform_(n_total / 1000, n_total / 100).item())
            sp = i0.new_empty(i0.size(0), 1, i0.size(2), i0.size(3)).bernoulli_(p=1-missing_rate).repeat(1, 1, 1, 1)

        hd = sp * i0
        hd_h = self.conv_hd(hd)
        sp_h = self.conv_hd(sp)
        # self.visualize(save_path + "hd_h", hd_h)
        # self.visualize(save_path + "sp_h", sp_h)
        hd_fmap = torch.cat([hd_h, sp_h], 1)

        # -----  First Level --------
        down_1_0 = self.down_1_0(hd_fmap)
        down_1_1 = self.down_1_1(i1)
        # self.visualize(save_path + "down_1_0", down_1_0)
        # self.visualize(save_path + "down_1_1", down_1_1)
        # -----  Second Level --------
        # input_2nd = torch.cat((down_1_0,down_1_1,down_1_2,down_1_3),dim=1)
        # Max-pool
        down_1_0m = self.pool_1_0(down_1_0)
        down_1_1m = self.pool_1_1(down_1_1)
        # self.visualize(save_path + "down_1_0m", down_1_0m)
        # self.visualize(save_path + "down_1_1m", down_1_1m)
        ...
        down_2_0m = self.pool_2_0(down_2_0)
        down_2_1m = self.pool_2_1(down_2_1)
        # self.visualize(save_path + "down_2_0m", down_2_0m)
        # self.visualize(save_path + "down_2_1m", down_2_1m)
        ...
        down_3_0m = self.pool_3_0(down_3_0)
        down_3_1m = self.pool_3_1(down_3_1)
        # self.visualize(save_path + "down_3_0m", down_3_0m)
        # self.visualize(save_path + "down_3_1m", down_3_1m)
        # ----------------------------------------
        # fusion layer
        down_fu_1 = self.down_fu_1(down_1_0m, down_1_1m)
        down_fu_1m = self.pool_fu_1(down_fu_1)
        self.visualize(save_path + "down_fu_1", down_fu_1m)
        down_fu_2 = self.down_fu_2(down_2_0m, down_2_1m, down_fu_1m)
        down_fu_2m = self.pool_fu_2(down_fu_2)
        self.visualize(save_path + "down_fu_2", down_fu_2m)
        down_fu_3 = self.down_fu_3(down_3_0m, down_3_1m, down_fu_2m)
        down_fu_4 = self.down_fu_4(down_fu_3)
        self.visualize(save_path + "down_fu_3", down_fu_3)
        self.visualize(save_path + "down_fu_4", down_fu_4)
        # latents     = self.down_fu_4(output_atten)
        # ######################################################################
        # ~~~~~~ Decoding
        deconv_1_0 = self.deconv_1_0(down_fu_4)
        self.visualize(save_path + "deconv_1_0", deconv_1_0)
        deconv_2_0 = self.deconv_2_0(down_3_0m, down_3_1m, deconv_1_0)
        self.visualize(save_path + "deconv_2_0", deconv_2_0)
        deconv_3_0 = self.deconv_3_0(down_2_0m, down_2_1m, up(deconv_2_0))
        self.visualize(save_path + "deconv_3_0", deconv_3_0)
        deconv_4_0 = self.deconv_4_0(down_1_0m, down_1_1m, up(deconv_3_0))
        self.visualize(save_path + "deconv_4_0", deconv_4_0)
        deconv_5_0 = self.deconv_5_0(up(deconv_4_0))
        self.visualize(save_path + "deconv_5_0", deconv_5_0)
        output = self.out(deconv_5_0)
        output[output > 0.9] = 1
        output[output <= 0.9] = 0
        self.visualize(save_path + "output", output)
        return output, sp, i1,

    def visualize(self, feature_name, feature_map):
        feature_map = feature_map.squeeze(0)
        feature_map = feature_map.cpu().detach().numpy()
        feature_map_num = feature_map.shape[0]
        row_num = numpy.ceil(numpy.sqrt(feature_map_num))
        final_map = feature_map[0]
        for idx in range(1, feature_map_num+1):
            final_map = final_map + feature_map[idx-1]
        # final_map = final_map / feature_map_num
        # plt.subplot(int(row_num), int(row_num), idx)
            plt.imshow(feature_map[idx-1], cmap="jet")
            plt.axis('on')
            plt.imsave("visualize_all_fmaps/" + feature_name + "_feature" + str(idx) + ".png", feature_map[idx-1], cmap="jet")
        # plt.show()
