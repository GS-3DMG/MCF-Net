...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf):
        ...
        self.conv_hd = nn.Conv3d(self.in_dim, self.out_dim, 5, 1, 2)

        #######################################################################
        # Encoder features of hard data
        #######################################################################
        self.down_1_0 = nn.Sequential(
            nn.Conv3d(in_channels=2 * self.out_dim, out_channels=self.out_dim, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, inputs):
        ...
        sp = i0.new_empty(i0.size(0), 1, i0.size(2), i0.size(3), i0.size(4)).bernoulli_(p=num_context / n_total).repeat(1, 1, 1, 1, 1)
        hd = sp * i0
    ...
    def complete(self, inputs, sp=None, missing_rate=None):
        ...
        if sp is None:
            if missing_rate is None:
                ...
                sp = i0.new_empty(i0.size(0), 1, i0.size(2), i0.size(3), i0.size(4)).bernoulli_(
                    p=1 - missing_rate).repeat(
                    1, 1, 1, 1, 1)

        # b, c, zz, yy, xx = i0.cpu().detach().numpy().shape
        # sp_idx_list = list(numpy.arange(xx*yy))
        # shuffle(sp_idx_list)
        # sp = torch.zeros_like(i0)
        # for i in range(0, 20):
        #     yi = int(sp_idx_list[i] / xx)
        #     xi = int(sp_idx_list[i] - yi*xx)
        #     for z in range(zz):
        #         sp[:, :, z, yi, xi] = 1
